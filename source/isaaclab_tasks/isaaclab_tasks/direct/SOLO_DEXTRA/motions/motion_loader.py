...
class MotionLoader:
    ...
    def __init__(self, motion_file: str, device: torch.device, speed_scale: float = 1.0) -> None:
        ...
        Args:
            ...
            speed_scale: Time stretch factor. >1 = slower (longer duration), <1 = faster.
                Velocity channels are automatically scaled by 1/speed_scale.
        ...
        # Scale dt by speed_scale: stretches effective duration while keeping same keyframes
        self.dt = (1.0 / data["fps"]) * self.speed_scale
        self.num_frames = self.dof_positions.shape[0]
        self.duration = self.dt * (self.num_frames - 1)

        # Scale velocity data: stretched time → slower velocities
        if self.speed_scale != 1.0:
            inv_scale = 1.0 / self.speed_scale
            self.dof_velocities = self.dof_velocities * inv_scale
            self.body_linear_velocities = self.body_linear_velocities * inv_scale
            self.body_angular_velocities = self.body_angular_velocities * inv_scale

        original_duration = (1.0 / data["fps"]) * (self.num_frames - 1)
        print(
            f"Motion loaded ({motion_file}): "
            f"original: {original_duration:.2f}s, "
            f"speed_scale: {self.speed_scale}, "
            f"effective duration: {self.duration:.2f}s, "
